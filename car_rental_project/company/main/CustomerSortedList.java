...
public class CustomerSortedList extends SortedList{
	
	public CustomerSortedList() {
		super();
	}
	...
	public void printItemsInHierarchy(String className){
		...
		try{
			while (tmp!=null){
				...
				if (Class.forName("tuc.ece.cs102.company.model.Customer." + className).isInstance(item.getData())){
					item.print();
				}
				tmp = tmp.getNext();
			}
