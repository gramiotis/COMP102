...
public class RentalSortedList extends SortedList{
	
	public RentalSortedList() {
		super();
	}
	...
	public void printItemsInHierarchy(String className){
		...
		try{
			while (tmp!=null){
				...
				if (Class.forName("tuc.ece.cs102.company.model.Rentals." + className).isInstance(item.getData())){
					item.print();
				}
				tmp = tmp.getNext();
			}
